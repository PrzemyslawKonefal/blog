...
import {minToReadText, polishMonthMapping} from "../../../../utils";
...
const ArticlesList = ({
  ...
  const hasOnlyOneArticle = articles && articles.length === 1;
   ...
   */
  const articlePairs = articles && articles.reduce((result, value, index, array) => {
    if (index % 2 === 0) {
...
const ListItem = ({ article, narrow }) => {
  const { gridLayout } = useContext(GridLayoutContext);
  const hasOverflow = article && narrow && article.title.length > 35;
  const imageSource = article && (narrow ? article.hero.narrow : article.hero.regular);
  const hasHeroImage =
    imageSource &&
    ...
    <ArticleLink to={article.slug} data-a11y="false">
      <Item gridLayout={gridLayout}>
        ...
        <div>
          ...
          <MetaData>
            {polishMonthMapping(article)} · {article.timeToRead} {minToReadText(article)} czytania
          </MetaData>
        </div>
      </Item>
    </ArticleLink>
