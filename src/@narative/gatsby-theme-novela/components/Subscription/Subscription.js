...
const mailchimpErrorTransformer = (error) => {
  const ALREADY_SUBSCRIBED_REGEX = /already subscribed/g
  const NOT_VALID_REGEX = /not valid/g;

  if (ALREADY_SUBSCRIBED_REGEX.test(error.msg)) return 'Twój email już jest na liście.';
  if (NOT_VALID_REGEX.test(error.msg)) return 'Email jest nieprawidłowy.';
  return error.msg
}
  ...
  function handleSubmit(event) {
    ...
    addToMailchimp(email)
      ...
      .catch(error => {
        ...
        setError(mailchimpErrorTransformer(error));
      });
  }
    ...
    <Section narrow>
      <SubscriptionContainer>
        <Content>
          ...
          <Text>
            Może któregoś dnia Ty lub ja zdecydujemy zaprzestać korzystania z mediów społecznościowych.
            Jeśli chcesz pomimo to pozostać w kontakcie i natychmiastowo dowiadywać się o nowych publikacjach - podrzuć swój email poniżej.
            Obiecuję nie spamować ani nie udostępniać danych podmiotom trzecim. :)
          </Text>
          <Form onSubmit={handleSubmit} hasError={error}>
            ...
            <Button
              ...
              disabled={subscribed}
            >
              {subscribed ? <CheckMarkIcon /> : "Dołącz"}
            </Button>
